...
RxData.Data : 00 9f
 ...
 1 6 0 7c 0 9f 8 7a
    ...
    inline errno_t MoveToSensor(uint8_t node_id, char xin, char level = 'F', bool is_cw = true)
    {
      if (m_Isconnected == false)
        return -1;

      if (writeMoveDirection(node_id, is_cw) == ERROR_SUCCESS)
      {
        // FS4L
        constexpr uint16_t feed_to_sensor = 0x006B;
        uint8_t func = write_MultiReg;
        uint16_t regist_no = Command_Opcode;
        uint16_t value = feed_to_sensor;
        enum { opcode, param1, param2, v_max };
        std::array<uint16_t, v_max> values = {
            value,
            (uint16_t)4,
        ...
        enum{id, fn, reg_h, reg_l, reg_cnt_h, reg_cnt_l,
          b_cnt, v5, v4, v3, v2, v1, v0, crc_l, crc_h,_max };
        uint16_t crc = 0xffff;
        std::array<uint8_t, _max> send_data = {
            node_id,
            func,
        ...
        /*logPrintf("MoveToSensor Packet : ");
            for ( auto& e:send_data)
            {
              logPrintf("0x%02X ", e);
            }
            logPrintf("\n");*/
      ...
      LOG_PRINT("MoveToSensor fail! node_id[%d]", node_id);
      return -1;
    }
  ...
  private:
    ...
    inline errno_t writeMoveDirection(uint8_t node_id, bool is_cw = true)
    {
      if (m_Isconnected == false)
        return -1;
      ...
      uint8_t  func = write_MultiReg;
      uint16_t start_reg_no = Point_to_Point_Distance_DI_D;
      enum { dist_h, dist_l, v_max };
      constexpr uint16_t reg_cnt = v_max;
      constexpr uint8_t byte_cnt = reg_cnt * 2;
      constexpr int direction_base_value = 1000;
      std::array<uint16_t, reg_cnt> value = {
          (uint16_t)((is_cw ? direction_base_value : direction_base_value * -1) >> 16),  //direction
          (uint16_t)((is_cw ? direction_base_value : direction_base_value * -1) >> 00)
      };
      ...
      /*logPrintf("writeMoveDirection Packet : ");
            for ( auto& e:send_data)
            {
              logPrintf("0x%02X ", e);
            }
