...
namespace MOTOR
{
	...
	public:
		...
		inline errno_t MoveToLimit(char level = 'F', bool is_ccw = true){
		  if (SetParamDataMove(m_cfg.motor_param.move_speed, m_cfg.motor_param.move_accel, m_cfg.motor_param.move_decel) == ERROR_SUCCESS)
		  {
		    if (!IsLimitPos(is_ccw))
		    {
		      return m_cfg.ptr_comm->MoveToSensor(m_nodeId, (is_ccw?m_cfg.origin_param.ccw_x_no: m_cfg.origin_param.ccw_x_no), level, !is_ccw);
		    }
		    else
		    {
		      LOG_PRINT("IsLimitPos(is_ccw) true");
		      return ERROR_SUCCESS;
		    }
		  }
		  else
		  {
		    LOG_PRINT("SetParamDataMove fail");
		  }
		  return -1;
		}

		inline bool IsLimitPos(bool is_ccw = true)	{ //b active
		  /*  ccw sensor 4, cw sensor 3   */
		  LOG_PRINT("-driver_board_inputs[%d] )",m_motorData.driver_board_inputs );
		  if (is_ccw)
		    return (bool)((m_motorData.driver_board_inputs >> 3) & 1); //b off state =  11111 0111
		  else
		    return (bool)((m_motorData.driver_board_inputs >> 2) & 1); //b off state =  11111 1011
		}
