...
public:
  ...
  inline void AlarmAuto(log_dat::head_t& head,
    const char* file,
    const char* func,
    const int line,
    const char* msg)
  {
    ...
    using err_t = ap_err_st::err_e;
    err_t err = static_cast<err_t>(head.error_no);
    ...
    LOG_PRINT("file[%s],func[%s-%d],msg[%d-%d-%s]", arg_tbl[arg_cnt - 1], func, line, head.error_no, head.obj_id, msg);
    AlarmAuto(err);
