...
void api_remote::ProcessCmd(RCTRL::uart_remote::packet_st& data)
{
  ...
  switch (rx_cmd)
  {
    ...
    case cmd_t::CMD_CTRL_MOT_ORIGIN:
      //m_cfg.ptr_task->MotorOrigin(constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX));
      break;

    case cmd_t::CMD_CTRL_MOT_ONOFF:
      Ret_OkResponse();
      if(m_cfg.ptr_motors->MotorOnOff( (bool)(m_receiveData.data[0]==1), (AP_OBJ::MOTOR)m_receiveData.obj_id) != ERROR_SUCCESS)
        LOG_PRINT("CMD_CTRL_MOT_ONOFF MotorOnOff failed!");
      break;

    case cmd_t::CMD_CTRL_MOT_MOVE:
      m_cfg.ptr_motors->Move(
          constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX),
          *(reinterpret_cast<int*>(&m_receiveData.data[0])));
      break;

    case cmd_t::CMD_CTRL_MOT_STOP:
      m_cfg.ptr_motors->MotorMoveStop(constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX));
      break;

    case cmd_t::CMD_CTRL_MOT_JOG:
              m_cfg.ptr_motors->MotorJogMove(constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX),!(m_receiveData.data[0]==0));
      break;

    case cmd_t::CMD_CTRL_MOT_LIMIT:
      m_cfg.ptr_motors->MoveToLimit(constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX));
      break;

    case cmd_t::CMD_CTRL_MOT_ZEROSET:
      m_cfg.ptr_motors->MotorZeroEncode(constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX));
      break;

    case cmd_t::CMD_CTRL_MOT_RELMOVE:
      m_cfg.ptr_motors->RelMove(
          constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX),
          *(reinterpret_cast<int*>(&m_receiveData.data[0])));
      break;

    case cmd_t::CMD_CTRL_MOT_CLEAR_ALARM:
      m_cfg.ptr_motors->MotorClearAlarm(constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX));
      break;
    ...
    case cmd_t::CMD_CTRL_MOT_MOVE_VEL:
      m_cfg.ptr_motors->Move(
          constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX),
          *(reinterpret_cast<int*>(&m_receiveData.data[0])),
          (uint32_t)*(reinterpret_cast<uint16_t*>(&m_receiveData.data[4])),
          (uint32_t)*(reinterpret_cast<uint16_t*>(&m_receiveData.data[6])),
          (uint32_t)*(reinterpret_cast<uint16_t*>(&m_receiveData.data[6])));
      break;
    ...
    case cmd_t::CMD_CTRL_MOT_VEL_JOG:
    {
      ...
      param.speed = *(reinterpret_cast<uint16_t*>(&m_receiveData.data[0]));
      param.accel = *(reinterpret_cast<uint16_t*>(&m_receiveData.data[2]));
      param.decel = *(reinterpret_cast<uint16_t*>(&m_receiveData.data[4]));
      bool is_cw =  (*(reinterpret_cast<int*>(&m_receiveData.data[6]))) > 0 ;
      m_cfg.ptr_motors->MotorJogMove(
          constrain((AP_OBJ::MOTOR)m_receiveData.obj_id,AP_OBJ::MOTOR(0),AP_OBJ::MOTOR::MOTOR_MAX),
          param, is_cw);
    }
    break;
